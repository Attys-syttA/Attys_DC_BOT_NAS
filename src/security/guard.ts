...
export function isAllowedRole(roleIds: Iterable<string>): boolean {
  const config = getConfig();
  if (config.ALLOWED_ROLE_IDS.length === 0) return false;
  for (const roleId of roleIds) {
    if (config.ALLOWED_ROLE_IDS.includes(roleId)) return true;
  }
  return false;
}

export function isAllowedPrincipal(userId: string, roleIds: Iterable<string> = []): boolean {
  return isAllowedUser(userId) || isAllowedRole(roleIds);
}
