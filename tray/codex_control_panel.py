#!/usr/bin/env python3
"""Attys DC BOT - Linux control panel."""
...
AUTOSTART_FILE = Path.home() / ".config" / "autostart" / "attys-dc-bot-tray.desktop"
USAGE_CACHE_PATH = Path.home() / ".codex" / "rate-limits-cache.json"
PANEL_PID_FILE = BOT_DIR / ".panel.pid"
SERVICE_NAME = "attys-dc-bot"
UPDATE_REPO = "https://api.github.com/repos/Attys-syttA/Attys_DC_BOT/releases/latest"
USAGE_URL = "https://chatgpt.com/codex/settings/usage"
GITHUB_URL = "https://github.com/Attys-syttA/Attys_DC_BOT"
ISSUES_URL = "https://github.com/Attys-syttA/Attys_DC_BOT/issues"
...
def is_running() -> bool:
    try:
        result = subprocess.run(
            ["/bin/bash", str(BOT_DIR / "linux-start.sh"), "--status"],
            cwd=str(BOT_DIR),
            capture_output=True,
            text=True,
            timeout=5,
...
def run_launcher(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        ["/bin/bash", str(BOT_DIR / "linux-start.sh"), *args],
        cwd=str(BOT_DIR),
        capture_output=True,
        text=True,
    )
...
def check_for_updates() -> None:
    ...
    try:
        ...
        if update_available:
            req = urllib.request.Request(UPDATE_REPO, headers={"Accept": "application/vnd.github.v3+json", "User-Agent": "attys-dc-bot-linux-panel"})
...
"""
    ...
    try:
        _send_json_line(proc, {
            ...
            "method": "initialize",
            "params": {
                "clientInfo": {"name": "attys-dc-bot-linux-panel", "version": current_version},
...
class ControlPanel:
    ...
    def refresh_ui(self) -> None:
        ...
        title = tk.Label(header, text="Attys DC BOT", font=("Helvetica", 20, "bold"), bg=BG_DARK, fg=FG_WHITE)
