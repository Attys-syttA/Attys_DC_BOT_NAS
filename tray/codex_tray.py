#!/usr/bin/env python3
"""Attys DC BOT - Linux System Tray App"""
...
try:
    ...
    sys.stderr.write(
        "Missing Linux tray dependencies. Install them explicitly with:\n"
...
def is_running():
    try:
        result = subprocess.run(
            ["/bin/bash", os.path.join(BOT_DIR, "linux-start.sh"), "--status"],
            cwd=BOT_DIR,
            capture_output=True,
            text=True,
            timeout=5,
...
def run_launcher(*args):
    return subprocess.run(
        ["/bin/bash", os.path.join(BOT_DIR, "linux-start.sh"), *args],
        cwd=BOT_DIR,
        capture_output=True,
    )
...
def fetch_release_notes():
    ...
    try:
        url = "https://api.github.com/repos/Attys-syttA/Attys_DC_BOT/releases"
        req = urllib.request.Request(url)
        req.add_header("Accept", "application/vnd.github.v3+json")
        req.add_header("User-Agent", "attys-dc-bot-tray")
...
def perform_update(icon, item):
    """Attys keeps Linux update actions read-only in this phase."""
    icon.notify(
        L(
...
def auto_rebuild_if_needed():
    """Kept for source compatibility; linux-start.sh owns build checks."""
    return
...
def open_github(icon, item):
    webbrowser.open("https://github.com/Attys-syttA/Attys_DC_BOT")


def open_github_issues(icon, item):
    webbrowser.open("https://github.com/Attys-syttA/Attys_DC_BOT/issues")
...
AUTOSTART_FILE = os.path.join(AUTOSTART_DIR, "attys-dc-bot-tray.desktop")
...
Type=Application
Name=Attys DC BOT Tray
Comment=Attys DC BOT system tray manager
...
"""
    ...
    try:
        _send_json_line(proc, {
            ...
            "method": "initialize",
            "params": {
                "clientInfo": {"name": "attys-dc-bot-tray", "version": current_version},
...
def _show_control_panel_gtk(icon):
    ...
    def rebuild():
        ...
        title_label.set_markup("<b><big>Attys DC BOT</big></b>")
        ...
        gh_link = Gtk.LinkButton.new_with_label(
            ...
            "GitHub: Attys-syttA/Attys_DC_BOT")
    ...
    win = Gtk.Window(title="Attys DC BOT")
    win.set_default_size(440, -1)
    win.set_position(Gtk.WindowPosition.CENTER)
    win.set_border_width(12)
    win.set_resizable(False)
    win.set_wmclass("attys-dc-bot-bot", "Attys DC BOT")
